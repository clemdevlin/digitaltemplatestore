...
interface AppContextType {
  ...
  getTransactionByToken: (
    token: string
  ) => Promise<{ transaction: Transaction | null; product: Product | null }>;
...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  const addProduct = async (productData: NewProduct) => {
    ...
    const { error: thumbError } = await supabase.storage
      .from("thumbnails")
      .upload(thumbPath, productData.thumbnailFile);
    if (thumbError)
    ...
    const { error: fileError } = await supabase.storage
      .from("templates")
      .upload(filePath, productData.productFile);
    if (fileError)
      throw new Error(`Product file upload failed: ${fileError.message}`);
  ...
  const deleteProduct = async (productId: string) => {
    ...
    if (productToDelete.file_path) {
      await supabase.storage
        .from("templates")
        .remove([productToDelete.file_path]);
    }
  ...
  const getSignedUrl = async (filePath: string) => {
    ...
    const { data, error } = await supabase.storage
      .from("templates")
      .download(filePath); // 60 seconds validity
    if (error || !data) {
